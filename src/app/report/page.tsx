  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      try {
        ...
        .replace(/```json\s*/i, '')  // Remove starting ```json (if present)
        .replace(/```$/, '')         // Remove ending ```
        .replace(/```/g, '')         // Remove any remaining backticks
        .trim();     
        // First parse to remove the outer quotes
        console.log("After first parse:", cleanedText);
        const firstParse = JSON.parse(cleanedText);


        // If after first parse it's still a string, parse again
        const parsedResult =
